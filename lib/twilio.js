 ...
 */
export async function sendWhatsAppMessage(to, message, mediaUrls = []) {
  try {
 ...
 * @param {Object} variables - Template variables (e.g., {1: 'value', 2: 'value'})
 * @returns {Promise<{success: boolean, sid?: string, error?: string}>}
 */
export async function sendWhatsAppTemplateMessage(to, templateSid, variables) {
  try {
    const fromNumber = process.env.TWILIO_WHATSAPP_NUMBER;
    if (!fromNumber) {
      throw new Error('TWILIO_WHATSAPP_NUMBER not configured');
