 ...
 * @param {string|Date} receivedAt - When the email was received (used as reference date for relative dates)
 * @returns {Promise<{success: boolean, reservations?: Array, error?: string}>}
 */
export async function parseReservations(emailBody, senderEmail, senderName = null, receivedAt = null) {
  try {
    // Use the email's received date as reference, not the current processing date.
    // This prevents errors when emails are processed later (e.g., during Thursday cron)
    // and relative dates like "próximo lunes" shift meaning.
    const referenceDate = receivedAt ? new Date(receivedAt) : new Date();
    const timeZone = 'America/Santiago';
    const todayStr = referenceDate.toLocaleDateString('es-ES', {
      weekday: 'long',
      year: 'numeric',
      month: 'long',
      day: 'numeric',
    ...
    // Build ISO date string in Santiago timezone
    const todayISO = referenceDate.toLocaleDateString('en-CA', { timeZone });
...
IMPORTANTE:
- Usa SIEMPRE el año actual (${referenceDate.getFullYear()}) a menos que se especifique otro año
- "Próximo miércoles" = el próximo día miércoles después de hoy
- "Mañana" = el día siguiente a ${todayISO}
